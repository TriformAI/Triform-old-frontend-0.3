<script lang="ts">
	...
	import SearchModals from './old-modals/SearchModals.svelte'
	import CanvasDropDownModal from './old-modals/CanvasDropDownModal.svelte'
	import FreeFormAutoArrangeModal from './old-modals/FreeFormAutoArrangeModal.svelte'
	import ComponentsToolbox from './old-modals/ComponentsToolbox.svelte'
	import EnvironmentModal from './old-modals/EnvironmentModal.svelte'
	import TokenModal from './old-modals/TokenModal.svelte'
	import StorageModal from './old-modals/StorageModal.svelte'
	import TemplateLibaryModal from './old-modals/TemplateLibaryModal.svelte'
	import PropertyModal from './old-modals/PropertyModal.svelte'
	import ConsoleModal from './old-modals/ConsoleModal.svelte'
...
<div
	class="flex items-center justify-between w-full py-0 border-b px-7 border-b-brand-primary-gray text-brand-white bg-website-primary"
>
	...
	<div class="relative flex items-center gap-x-3 left-10">
		{#each icons.filter(icon => icon.visibleOnToolbar) as { id, icon, alt, modalComponent }, index}
			...
			>
				...
				{#if $canvasToolsModal}
					<button class="absolute inset-y-0 right-0 top-7" onclick={() => toggleIconVisibility(id)}>
	...
	<div class="flex items-center gap-x-4">
		...
		<div class="px-5 border-x-2 border-x-brand-primary-gray">
			<div class="p-1.5 cursor-pointer bg-website-tertiary rounded-xl">
				<img src={grid_icon} alt="grid_icon" class="w-[1.15rem]" />
			</div>
		</div>
		...
		>
			<img src={draw_icon} alt="draw_icon" class="w-[1.15rem]" />
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
				viewBox="0 0 24 24"
		...
		>
			<div class="p-2 cursor-pointer hover:bg-website-tertiary rounded-xl">
				<img src={search_icon} alt="search_icon" class="w-[1.15rem]" />
			</div>
		</button>
	</div>
</div>
